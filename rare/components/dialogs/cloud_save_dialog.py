...
class CloudSaveDialog(QDialog, Ui_SyncSaveDialog):
    ...
    def __init__(
        ...
        self.sync_widget_ui = Ui_SyncWidget()
        self.sync_widget_ui.setupUi(self.sync_widget)
        ...
        self.sync_widget_ui.date_info_local.setText(dt_local.strftime("%A, %d. %B %Y %X") if dt_local else "None")
        self.sync_widget_ui.date_info_remote.setText(dt_remote.strftime("%A, %d. %B %Y %X") if dt_remote else "None")
        ...
        if newer == "remote":
            self.sync_widget_ui.cloud_gb.setTitle(self.sync_widget_ui.cloud_gb.title() + new_text)
        elif newer == "local":
            self.sync_widget_ui.local_gb.setTitle(self.sync_widget_ui.local_gb.title() + new_text)

        self.sync_widget_ui.icon_local.setPixmap(icon("mdi.harddisk", "fa.desktop").pixmap(128, 128))
        self.sync_widget_ui.icon_remote.setPixmap(icon("mdi.cloud-outline", "ei.cloud").pixmap(128, 128))

        self.sync_widget_ui.upload_button.clicked.connect(lambda: self.btn_clicked(self.UPLOAD))
        self.sync_widget_ui.download_button.clicked.connect(lambda: self.btn_clicked(self.DOWNLOAD))
